...
为了简化维护并符合项目最新要求：
- **位置**: 新文章始终通过 `push` 操作插入到 `contentItems` 数组的**最末端**。
- **新文章 ID**: 设为 `当前数组中最大 ID + 1` (例如：如果原有最大 ID 是 13，新文章 ID 即为 14)。
- **ID 稳定性**: 不再执行任何“ID 顺延”操作，确保已有文章的 ID 保持不变。
- **引用同步**: 直接使用目标文章的固定 ID 填充 `relatedIds` 和 `recommendationIds`，无需进行 `+1` 偏移计算。
...
## 5. 验证清单
- [ ] 确认新文章已添加至 `contentItems` 数组末尾且 ID 正确递增。
